...
> 本文是 `package/server_side` 的唯一基础设施交接规范。接手的 AI Agent 应先完整阅读本文，再按里程碑顺序实现；不要自行扩展当前范围。
>
...
## 0. 背景
这是一个企业用 RAG + AI Agent 一体的项目。`package/client_side` 负责用户对话管理、上传与查看 RAG 文件；`package/server_side` 负责认证授权、安全检索、LLM 通信、tool/skill call 与多 Agent 编排。用户只与服务端 `/root` Agent 对接。
...
- 由 `/root` 统一对接用户、支持动态树状委派与并行员工 Agent 的运行时；
- 受权限约束的 Tools、可通过 `/skill-name` 注入子 Agent 的 Skills；
...
以下能力不实现，**不创建占位模块、空接口、抽象基类、假实现或返回伪数据的路由**：

- 服务端对话历史、长期记忆与跨请求 Agent 团队恢复；
- SSE/WebSocket 流式输出；
- 后台或脱离当前 HTTP 请求继续运行的异步 Agent；
- 由模型访问 `data.directory/agent_work/<user_id>` 之外的文件系统、Shell、任意网络或数据库；
- 客户端自选模型、system prompt、Tools、Skills 或 Agent 权限。
...
flowchart TB
    ...
    App --> Runtime["Runtime：/root 与动态 Agent 团队"]
    ...
    Runtime --> LangGraph["LangChain Agent / LangGraph"]
    LangGraph --> Tools["受权限约束的 Tools"]
    Tools --> Knowledge
    Runtime --> Skills["只读内置 Skills"]
```
...
```text
http -> identity / access_control / knowledge / runtime / usage
runtime -> identity / access_control / knowledge / usage / LangChain / LangGraph
...
- `runtime` 独占 Agent 团队树、递归深度、并行委派、父子消息与 Skill 注入；它只通过正式应用服务使用用户权限和知识，不向模型开放 SQLite、LanceDB、对象目录或任意系统工具。
...
│  │  └─ service.py
│  ├─ llm/
│  │  ├─ openai_chat.py              # 保留兼容供应商的 reasoning_content
│  │  └─ openai_embeddings.py        # embeddings 的严格 HTTP 客户端
│  ├─ runtime/
│  │  ├─ context.py                 # 估算 token、滚动 checkpoint 与请求投影裁剪
│  │  ├─ files.py                   # 用户隔离 agent_work 与原子文件操作
│  │  ├─ file_tools.py              # 所有 Agent 默认继承的文件 Tool schema
│  │  ├─ models.py
│  │  ├─ service.py                  # /root、动态团队树与最终拍板
│  │  ├─ tools.py                    # 真实 Agent Tools schema
│  │  ├─ skill_registry.py           # 只读加载内置 SKILL.md
...
禁止创建没有真实调用方的 Agent/Tool/Skill 占位目录，也禁止 `providers/`、`interfaces/`、`migrations/` 等无调用方抽象。
...
└─ agent_work/<user_id>/            # 同一用户所有父子 Agent 的持久共享工作区
```
...
- embeddings 仍使用一个共享的 `httpx.AsyncClient` 直接调用 OpenAI 兼容协议并执行严格响应校验。
- Agent 采用 LangChain `create_agent`（底层 LangGraph）与 `ChatOpenAI` 的 Chat Completions API 集成。选择它是因为 Supervisor/Subagents 模式允许主 Agent 动态委派、同轮并行 tool calls 和多跳调用；团队层级、授权与 Skill 注入仍由本项目 `runtime.service` 控制。`OpenAICompatibleChatModel` 补充 LangChain 通用转换器遗漏的非标准 `reasoning_content`。供应商请求显式发送 `max_completion_tokens=output_tokens` 与 `thinking_budget_tokens=thinking_tokens`。
...
| `agent.model` | 是 | 服务端固定的 Agent chat model；客户端不可覆盖 |
| `agent.context_window_tokens` | 是 | 模型上下文窗口 |
| `agent.provider_concurrency` | 是 | 整个服务同时打到 embeddings/chat 供应商的 HTTP 请求数 |
| `agent.input_tokens` | 是 | 每次 Agent 模型调用允许的最大输入 tokens |
| `agent.output_tokens` | 是 | 每次调用的组合输出上限，包含思考与可见输出 |
| `agent.thinking_tokens` | 是 | 组合输出中允许用于思考的 tokens；必须小于 output |
| `agent.compression_threshold` | 否 | 估算可见上下文达到输入预算的比例时同步压缩；默认 `0.8`，必须满足 `0 < value < 1` |
| `agent.thinking_level_options` | 否 | 允许请求选择的思考等级字符串；默认 `[on, off, low, medium, high, xhigh, max]`，可自定义或设为空列表 |
| `agent.default_thinking_level` | 是 | 请求未指定时采用的等级；必须为空字符串（供应商默认）或存在于 options 中 |
...
| `limits.default_monthly_agent_tokens` | 是 | 用户每自然月 Agent chat token 限额 |

Agent model 由启动配置固定；temperature、system prompt、tool schema 和 Skill 来源由服务端代码固定，不接受客户端覆盖。`input_tokens + output_tokens` 必须不超过 context window；真正可见输出严格定义为 `output_tokens - thinking_tokens`。
...
9. 创建共享 `httpx.AsyncClient`、embedding client、固定 chat model、Skill registry、用户隔离的 `agent_work` service、Agent Runtime、其余 usage/knowledge services、router factories 和 FastAPI app；不使用 DI 容器。Skill registry 启动时完整校验随包发布的 `runtime/skills/*/SKILL.md`，不读取客户端路径。
...
| `user_limits` | `user_id PRIMARY KEY`, 文档/存储/embedding/Agent 四类可空覆盖限额, `updated_by_user_id`, `updated_at` | 空值使用启动默认值 |
| `usage_monthly` | `(user_id, month_utc) PRIMARY KEY`, `embedding_tokens`, `agent_tokens`, `search_requests`, `uploads`, `updated_at` | 月度聚合，不记录 prompt/chunk/回答 |
...
| `agent.run` | 调用 `/root` Agent 及其受控的动态员工团队 |
...
| `knowledge_manager` | agent run、user/role read、upload/read/update/delete/ACL manage/search、self usage；仍受文档 ACL 限制，可读取用户/角色目录以选择授权主体 |
| `contributor` | agent run、upload/read/update/search、self usage；仍受 ACL 限制 |
| `reader` | agent run、read/search、self usage；仍受 ACL 限制 |
...
| 运行 Agent | `agent.run`；Agent 内每个业务 Tool 仍独立检查其具体能力与文档 ACL |
...
7. 只有复核通过的 chunk、标题、文件元数据和引用可以返回客户端或送入 Agent LLM。

权限/ACL 撤销不改写向量。撤销对下一请求立即生效；同一搜索在返回内容前也要做第 6 步复核，尽量阻止请求执行期间发生的撤权继续披露。

### 8.7 Agent 团队与 Tools/Skills

用户只调用 `/root`。`/root` 自行判断任务复杂度和可并行性；简单任务直接完成，复杂任务通过 `delegate_tasks` 动态创建员工。团队不是预定义 DAG，也没有服务端写死的 todo list：

- 每个 Agent 都由 LangChain `create_agent` 构建，底层执行循环为 LangGraph；
- `delegate_tasks` 一次接受一个或多个自包含任务，同一调用内的独立任务使用 `asyncio.gather` 并行；
- 员工拥有同样的委派能力，因此可以继续创建孙 Agent，直到派生的最大递归深度；
- 运行时不接受额外的 team/depth/step/output 常量。设 `turn = input_tokens + output_tokens`、`slots = floor(context_window_tokens / turn)`，则员工总数与最大递归深度均为 `slots - 1`，单次委派宽度为 `min(provider_concurrency, slots - 1)`，LangGraph step limit 为 `floor(context_window_tokens / (output_tokens - thinking_tokens))`；
- embeddings client、`/root` 与所有子 Agent 共享一个进程级供应商 semaphore，因此并发 HTTP 请求、索引/搜索 embeddings 与同一团队内的 chat 调用合计都不会超过 `provider_concurrency`；
- 父 Agent 收到完成/失败报告后自行评估；可用 `message_agent` 对自己的任意已完成后代继续追问，后代保留本次 HTTP 请求内的消息历史；
- 子 Agent 报告只回到父 Agent。HTTP 响应只包含 `/root` 的最终回答、员工总数和达到的最大深度，不返回内部任务、报告、prompt 或 tool transcript；
- 团队与历史只存在于当前同步 HTTP 请求；当前不做后台 Agent、跨请求恢复或服务端对话记忆。

真实 Tools 固定为：

| Tool | 作用与边界 |
|---|---|
| `delegate_tasks` | 创建动态员工；任务文本中的已注册 `/skill-name` 会注入该员工 |
| `message_agent` | 向当前 Agent 的已完成后代发送后续任务；不能访问自身、祖先或兄弟树 |
| `search_knowledge` | 调用正式 `KnowledgeService.search`，保留 `rag.search + rag.document.read + ACL` 全链路 |
| `get_document_manifest` | 返回授权且 ready 文档的当前 hash、字符数、chunk 数和分页章节轮廓；章节识别对 PDF/DOCX 是启发式 |
| `read_document_text` | 按字符 offset 从 0 到 `eof=true` 连续读取授权文档的完整抽取文本；续读必须携带 manifest 的 hash |
| `read_chunk_context` | 按命中结果的 document ID、ordinal 与 hash 读取受限数量的前后 chunk |
| `search_document_text` | 对单份授权文档的完整抽取文本做穷尽式字面搜索；同时要求 `rag.search`，零结果不代表扫描图像中绝对不存在 |
| `read_document_section` | 按 manifest 返回的 section ID 与章节内字符 offset 连续读取；不接受文件路径 |
| `list_team` | 只返回后代 path、depth、status 与 skill 名，不返回私有任务历史 |
| `list_skills` | 只列已注册 Skill 的 name/description |
| `load_skill` | 按精确名称读取随包 Skill；不接受文件路径 |
| `write_file` / `read_file` / `read_files` | 在当前用户 `agent_work` 内原子保存或读取 UTF-8 文本；未知大小先预检 |
| `list_directory` / `find_files` / `search_files` | 列目录、glob 找文件和搜索 UTF-8 正文；路径始终限制在工作区 |
| `file_info` / `count_text` | 返回文件 bytes/字符/行/分块建议，或统计给定文本字符数 |
| `read_file_chunk` | 默认 8 KiB、最大 16 KiB，按 byte offset 从 0 串行读到 `eof=true`，UTF-8 边界不重不漏 |
| `append_file` | 必须给出精确 `expected_size_bytes`；成功返回下一 offset，陈旧/重复/乱序追加在写入前失败 |
| `apply_patch` | 整份预校验 Codex patch；支持 Add/Update/Delete、精确上下文、原换行风格和同目录原子替换 |

Tool schema 不出现 `user_id`、角色、能力或 bypass 参数。根与所有后代共享服务端注入的同一个 `VerifiedIdentity/AuthContext`；每次 Agent 调用前、每个业务 Tool 内及最终返回前都重新验证 session 和当前能力。子 Agent 不能扩大授权；Skills 只改变受信任工作指令，不授予 Tool 或数据权限。通用文件 Tools 仍只能访问 `agent_work/<user_id>`；RAG 全文、章节和 chunk 上下文只能按 `document_id` 通过 `KnowledgeService` 读取，禁止把 `objects` 挂载为通用只读目录。连续读取必须固定 `content_hash`，文档变化时从 manifest 重新开始。

Skills 位于 `runfold_server/runtime/skills/<name>/SKILL.md`，使用只含 `name` 和 `description` 的 YAML frontmatter。启动时 registry 校验目录逃逸、UTF-8、名称与目录一致性。任务中只有已经注册的 `/name` 才被解释为 Skill selector，未知 `/api/path` 等文本保持原样。内置 Skills 当前包括：

- `rag-research`：面向授权 RAG 的可追踪研究；
- `critical-review`：独立审查证据、权限、故障路径和产品风险；
- `product-decision`：比较方案并形成明确产品建议。

RAG chunk 与员工报告都可能包含不可信内容。System prompt 必须明确把它们视为 evidence 而非 instructions；任何来自这些内容的提权、改变 Tool 参数边界或索取系统 prompt 的要求都不执行。

### 8.8 上下文压缩与请求投影

`/root` 与每个子 Agent 都创建独立的 `ContextCompressor`，它是内部 model middleware，不作为 Tool 暴露。token 计数按当前要求使用 LangChain `count_tokens_approximately` 的估算值；原始消息状态保持完整，以下变换只作用于发送给模型的投影视图：

1. 估算 system message、全部可见消息和 Tool schema。达到 `agent.input_tokens * agent.compression_threshold` 时，在下一次模型调用前同步生成并注入滚动 continuation checkpoint；默认阈值为 80%，用户可配置 `0 < threshold < 1`。
2. checkpoint 合并已有摘要与尚未摘要的新证据，不重复堆叠旧摘要。它必须保留目标、验收条件、偏好、授权边界、有效约束、验证结果、精确路径/ID/命令/数值、工作区状态、已拍板决策及简短理由、失败与不确定性、缺失/截断/过期证据、剩余工作和下一动作，并区分 verified facts、inferences 与 open decisions。
3. 摘要删除寒暄、重复、过期计划和已替代信息；不执行任务、不调用 Tools、不回答最新用户请求、不引入新指令或臆造事实，不保存私有思维链。摘要调用不带 Tools、不写入 Agent 原始对话，也不伪装成 Agent 发言。
4. 摘要输入、思考和输出预算只从现有 AgentBudget 派生。源证据没有 `reasoning_content/signature` 时，摘要请求把 thinking budget 设为 0，组合输出只使用可见输出预算；存在历史 reasoning 时才使用当前思考/组合输出预算。摘要调用与正常调用共享供应商 semaphore、权限复核、月度额度和 token 记账。
5. 已完成的旧 tool-call 阶段只在投影视图移除 `reasoning_content` 与 `reasoning_signature`；Tool call、匹配 Tool result、assistant 可见文本和最新活动阶段保持完整，原始消息不变。`OpenAICompatibleChatModel` 在下一次 LM Studio Chat Completions 请求中把最新活动 assistant tool-call 的 reasoning/signature 原样序列化回消息，随后再附上匹配的 ToolMessage，避免破坏 Qwen 工具调用连续性。
6. 单个 Tool result 的估算 token 达到同一个 `agent.input_tokens * agent.compression_threshold` 触发值时，只在投影视图保留头尾和明确截断标记。这里不使用第二个比例或硬编码阈值。需要中段时必须从原始来源或 `agent_work` 重新串行分块读取，不能把截断结果当完整证据。
7. checkpoint 后仍超过 `agent.input_tokens` 时，只从最旧处删除完整历史阶段；Tool call 与匹配结果不拆开，最新 user/活动阶段不删除。若活动阶段自身仍超限，调用前以 `413 agent_input_token_limit` 安全失败。
8. Agent 应在压缩前把后续必须逐字使用的长证据或生成结果可靠写入 `agent_work`，验证成功并在报告/checkpoint 中记录精确路径、用途和读取方式；可无损概括的内容直接进入 checkpoint，无需机械落盘。
...
- 本模块不实现 chat、responses、streaming 或 provider interface；Agent chat 由 `runtime` 中固定的 LangChain model 集成负责，避免 embeddings 与 Agent 协议互相污染。
...
POST   /api/agent/runs                              # 用户只对接 /root
...
不创建公开 `/chat`、`/tools`、`/skills` 路由；Agent 只公开有真实实现的 `/api/agent/runs`。
...
`thinking_level` 可省略；省略时使用 `agent.default_thinking_level`，显式空字符串表示临时使用供应商默认值，非空值必须存在于 `agent.thinking_level_options`。同一请求的 `/root` 和所有后代使用同一等级。客户端不能提交 model、system prompt、tools、skills、递归深度、用户 ID 或权限。响应：

```json
{
  "answer": "/root 的最终回答",
  "reasoning_content": "供应商返回的 /root 最终思考内容；没有则为 null",
  "thinking_level": "high",
  "agents_created": 3,
  "max_depth_reached": 2
}
...
用户可见的 Agent 内容包括 `/root` 的 `answer` 与供应商返回的最终 `reasoning_content`。员工任务、员工报告和员工 reasoning 不出现在响应中。`reasoning_content` 不写入日志或审计。
...
- 用户每 UTC 自然月 Agent chat tokens：`/root` 与所有后代的模型调用均归属发起用户；
- 管理员可在 `user_limits` 覆盖默认文档数、存储字节、月 embedding tokens 和月 Agent tokens。

不要增加没有调用方的 tool 次数等字段。每个 embedding 调用/批次和每次 Agent invocation 只在调用前检查当时额度；调用后用供应商 usage 入账。当前已获准的调用不会因自身入账后使累计值超过限额而被最终事务再次否决，后续调用再拒绝。由于外部调用不能与 SQLite 事务绑定，一个已在途请求或同一 Agent graph 内的连续模型步骤可能小幅越过月额度；当前接受这一明确限制，不为严格预留引入 reservation 子系统或长事务。

`created_by_user_id` 同时是不可变的文档聚合计费归属。即使创建者后来失去文档 ACL，该文档仍计入其文档数和存储字节额度；创建者通过 `/api/usage/me` 有权看到自己承担的聚合 current/limit/remaining，但只能看到总文档数、总字节数、月 embedding tokens 和自己的月 Agent tokens，绝不返回隐藏文档的 ID、标题、状态、hash、类型或分项。这是独立的计费知情权限，而不是 READ ACL；因此本文的零披露承诺明确不包含创建者自己的聚合计费值。只有拥有根能力 `usage.all.read` 的 system_admin 可查看其他用户的完整聚合。
...
- Agent run 允许/拒绝、完成状态、员工总数和最大深度；不得保存用户 input、员工任务/报告、prompt、tool transcript 或最终回答；
- 配额拒绝与限额修改；
- 所有 bypass；
...
19. 日志、错误和审计中不存在 session token、密码、API key、正文、chunk、完整 query、Agent input、员工报告或最终回答。
20. `/root` 与所有后代收到同一个服务端 `AuthContext`；Tool schema 不接受身份/角色/能力/bypass，撤销 session 或 `agent.run` 后不得继续返回回答。
21. 动态树同时覆盖同级并行、员工创建孙 Agent、最大递归深度、单次员工总数、并行宽度与 LangGraph step limit。
22. 父 Agent 能继续消息已完成后代；不能访问自身、祖先或兄弟树；员工报告不直接出现在 HTTP 响应。
23. `/skill-name` 只注入已注册 Skill，未知 slash 路径保持普通文本，任意文件路径无法加载；检索内容不能改变授权或 system prompt。
24. `/root` 与全部后代的 Agent tokens 归属发起用户；额度用尽时在下一次外部调用前拒绝并审计。
25. 所有父子 Agent 默认获得完整文件 Tool 集；跨用户、绝对路径、`..` 与符号链接逃逸拒绝，chunk UTF-8 不重不漏，append 冲突不产生重复，patch 预校验失败不产生任何写入。
26. 估算上下文达到用户配置阈值时，同一轮下一次模型调用看到最新 checkpoint；旧 reasoning 清理、超大 Tool result 头尾截断和完整阶段 tailoring 只改变投影视图，原始消息保持完整。
27. RAG 文档 manifest、全文分块、章节和命中上下文读取始终经过 `rag.document.read + READ ACL + ready`；字面全文搜索还要求 `rag.search`；撤权、状态变化或 hash 变化后续调用立即失败，通用文件 Tool 不能读取对象目录。
...
### M6：Agent Runtime、Tools 与 Skills

交付：

- LangChain `create_agent` / LangGraph 执行循环与固定 OpenAI Chat Completions model；
- 用户唯一入口 `/api/agent/runs` 与 `/root` 最终拍板；
- 动态树状 `delegate_tasks`、同次调用并行、员工递归委派、父对后代 follow-up；
- 上下文、供应商并发、输入、组合输出和思考五项资源配置，以及由它们唯一派生的团队/图限制；
- `search_knowledge`、`get_document_manifest`、`read_document_text`、`read_chunk_context`、`search_document_text`、`read_document_section`、`list_team`、`list_skills`、`load_skill` 等真实 Tools；
- 所有 Agent 默认继承的用户隔离 `agent_work` 文件 Tool 套装与 Codex-style `apply_patch`；
- 可配置压缩阈值（默认 80%）、估算 token、同步滚动 checkpoint、旧 reasoning 清理、超大 Tool result 截断与完整阶段 tailoring；
- `rag-research`、`critical-review`、`product-decision` Skills 与 `/skill-name` 子 Agent 注入；
- `agent.run` 能力、全程 AuthContext 复核、月 Agent token 限额与脱敏审计；
- 删除临时无 Tools 的 Responses 透传路由及其旧测试。

验收：没有固定 DAG/todo；简单任务可由 `/root` 直接处理，复杂任务能按模型判断动态组队；子 Agent 权限不超过发起用户；达到深度/数量/并行/step/token 限制时安全收敛；用户只收到 `/root` 最终回答。
...
- **Agent prompt injection 边界**：RAG 与员工报告一律作为不可信 evidence，并在固定 system prompt 中禁止把它们当权限或工具指令；真正的安全边界仍是服务端 AuthContext、能力检查、文档 ACL 和无身份参数的 Tool schema，不能声称仅靠 prompt 可消除注入风险。
...
- [LangChain Multi-agent Subagents：Supervisor、并行与上下文隔离](https://docs.langchain.com/oss/python/langchain/multi-agent/subagents)
- [LangChain Skills：渐进披露与层级 Skills](https://docs.langchain.com/oss/python/langchain/multi-agent/skills)
- [LangGraph Graph API：运行时 recursion limit](https://docs.langchain.com/oss/python/langgraph/graph-api)
