...
Tool schema 不出现 `user_id`、角色、能力或 bypass 参数。根与所有后代共享服务端注入的同一个 `VerifiedIdentity/AuthContext`；每次 Agent 调用前、每个业务 Tool 内及最终返回前都重新验证 session 和当前能力。子 Agent 不能扩大授权；Skills 只改变受信任工作指令，不授予 Tool 或数据权限。
...
- `search_knowledge`、`list_team`、`list_skills`、`load_skill` 等真实 Tools；
