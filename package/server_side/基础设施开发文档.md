...
5. **默认拒绝与零披露**
   ...
   - 未显式允许即拒绝。无权限时必须在读取对象文件、抽取文本或查询向量之前停止。
   - 任何性能优化都不得退化成“先搜索全库，再在 Python 里过滤”。
...
| LLM/embedding 密钥 | 进程启动配置 | 不写入数据库、响应、日志或审计 |
...
├─ runfold_server/                   # 直接可由 python -m runfold_server 启动，不需要产品版本元数据
│  ├─ __init__.py
│  ├─ __main__.py                    # 唯一入口；serve/维护命令仍属于同一入口
│  ├─ bootstrap.py                   # 唯一手工装配点
│  ├─ config.py                      # 环境变量解析与启动校验
...
| 配置 | 必需 | 含义 |
|---|---:|---|
| `RUNFOLD_HOST` | 否 | 监听地址，默认仅本机；生产由部署显式配置 |
| `RUNFOLD_PORT` | 否 | 监听端口 |
| `RUNFOLD_DATA_DIR` | 是 | SQLite、objects、LanceDB、staging 的唯一数据根目录 |
| `RUNFOLD_ALLOWED_ORIGINS` | 是 | 精确 CORS origin 列表；带认证时禁止 `*` |
| `RUNFOLD_OPENAI_BASE_URL` | 是 | OpenAI 兼容根地址，规范化后包含供应商要求的 `/v1`；该 `/v1` 是外部协议路径，不是本项目 API 版本 |
| `RUNFOLD_OPENAI_API_KEY` | 是 | 可显式为空以支持本地兼容服务；非空时发送 Bearer |
| `RUNFOLD_EMBEDDING_MODEL` | 是 | 当前索引唯一使用的 embedding 模型 |
| `RUNFOLD_EMBEDDING_DIMENSIONS` | 是 | 期望向量维度；每个响应都要校验 |
| `RUNFOLD_EMBED_BATCH_SIZE` | 是 | 单次 `/embeddings` 的 chunk 数 |
| `RUNFOLD_LLM_TIMEOUT_SECONDS` | 是 | 连接与响应超时 |
| `RUNFOLD_LLM_MAX_RETRIES` | 是 | 对网络错误、429、502/503/504 的有限重试次数 |
| `RUNFOLD_CHUNK_SIZE` | 是 | 规范化字符分块大小 |
| `RUNFOLD_CHUNK_OVERLAP` | 是 | 相邻块重叠；必须小于 chunk size |
| `RUNFOLD_UPLOAD_MAX_BYTES` | 是 | 单文件上传上限 |
| `RUNFOLD_EXTRACT_MAX_CHARACTERS` | 是 | 单文档抽取文本上限 |
| `RUNFOLD_PDF_MAX_PAGES` | 是 | PDF 页数上限 |
| `RUNFOLD_DOCX_MAX_UNCOMPRESSED_BYTES` | 是 | DOCX 解压总量上限 |
| `RUNFOLD_SESSION_TTL_SECONDS` | 是 | session 有效期 |
| `RUNFOLD_BOOTSTRAP_ADMIN_USERNAME` | 空库必需 | 仅空用户库首次启动创建管理员 |
| `RUNFOLD_BOOTSTRAP_ADMIN_PASSWORD` | 空库必需 | 首次管理员密码，不记录、不回显 |
| `RUNFOLD_DEFAULT_MAX_DOCUMENTS` | 是 | 用户默认当前文档数限额 |
| `RUNFOLD_DEFAULT_MAX_STORAGE_BYTES` | 是 | 用户默认当前对象字节限额 |
| `RUNFOLD_DEFAULT_MONTHLY_EMBEDDING_TOKENS` | 是 | 用户每自然月 embedding token 限额 |

当前没有 LLM chat 调用方，因此不要提前增加 chat model、temperature、system prompt 等未使用配置。密钥不得写入 SQLite、LanceDB、对象元数据、日志、审计或异常正文。
...
6. 创建正式 `IdentityService`、`AccessControlService` 和其审计依赖。全新数据库中由 `schema.sql` 写入固定能力与默认角色；用户表为空时，通过正式 service/repository 方法在同一事务创建首个用户、分配受保护 system_admin 角色并写审计，禁止在 `bootstrap.py` 内直接写密码或手写业务 SQL。成功后要求运维移除密码环境变量。
