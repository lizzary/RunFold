...
9. 创建共享 `httpx.AsyncClient`、embedding client、固定 chat model、Skill registry、Agent Runtime、其余 usage/knowledge services、router factories 和 FastAPI app；不使用 DI 容器。Skill registry 启动时完整校验随包发布的 `runtime/skills/*/SKILL.md`，不读取客户端路径。
...
Skills 位于 `runfold_server/runtime/skills/<name>/SKILL.md`，使用只含 `name` 和 `description` 的 YAML frontmatter。启动时 registry 校验目录逃逸、UTF-8、大小、名称与目录一致性。任务中只有已经注册的 `/name` 才被解释为 Skill selector，未知 `/api/path` 等文本保持原样。内置 Skills 当前包括：
...
客户端不能提交 model、system prompt、tools、skills、递归深度、用户 ID 或权限。响应：
...
`answer` 是唯一用户可见的 Agent 内容；员工任务和报告不出现在响应中。
